...
class VcodeController extends Controller{
    ...
    public function apply_sms(){
        ...
        $code = create_code(4);
        $datas = array($code,'5分钟');
        $this->_send_sms(Constants::SMS_APPLY_CODE, Constants::SMS_APPLY_VCODE, $datas);
     ...
     * @return [type]           [description]
     */
    public function image(){$vcode="1234";
	...
	public function getImgCode($vcode="1234") {  
		$code = $vcode;  
     ...
     * @return [type] [description]
     */
    public function new_web_reg_sms(){
        $mobile = I('mobile', '', 'addslashes');
        $is_reg = D('Members')->is_reg_vcode($mobile);
		$this->_web_valid();
        if($is_reg){
	...
	private function _web_valid(){
		...
		$validcode = I('imgcode', '', 'addslashes');
