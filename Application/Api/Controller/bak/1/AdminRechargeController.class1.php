    ...
    public function addchain(){
        ...
        foreach ($data as $k=>$v){
                ...
                if(empty($member) || !empty($findorder)){ 
                    ...
                    $pass = $pass && $succid = D("Recharge")->adds($order);
                    //充值余额
                    ...
                    $pass = $pass && D("Recharge")->changestate($v['ordersn'],$changeArr);     //加日志
                    $log = array(
                        "uid"=>$member['id'],
                        "changeform"=>"in",
                        "subtype"=>2,
			   ...
			   $findArr = M("recharge")->where(array('phone'=>$v['phone']))->field("ordersn")->select();var_dump($findArr);
			   for($i=0;$i<count($findArr);$i++){
				   $urldata = array(
				   'ordersn'=>$findArr[$i]['ordersn']
				   );
				   $url = "http://t1.renrengyw.com/Api/Chain/changestatus";
				   $this->httpPost($url,$urldata);
			   }
        }
