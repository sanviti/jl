...
class KeySetController extends BaseController{
 ...
 //添加KEY价格
    ...
    public function save_trading_amount(){
		...
		$count = $trading_amountModel->field('id,amount,ctime')->where($cond)->count();
		...
		if($count==0){
			...
			if($amount == 0){
				$firstdata = array(
					'amount'=>$amount,
					'ctime'=>$last_time+3600*24
				);
				M('trading_amount')->add($firstdata);
				$this->redirect('KeySet/add_coin_amount');
			}
			$where['ctime'] = array('lt',$last_time+3600*24);
			$find = $trading_amountModel->field('id,amount,ctime')->where($where)->order('ctime desc')->find();
