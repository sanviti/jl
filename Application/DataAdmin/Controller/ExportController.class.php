...
class ExportController extends BaseController{
    ...
    public function apply_export(){
        ...
        $data = M("applycash")->alias("a")->where($condi)
                ->field("a.id,a.money,a.account,a.disburse,
                         a.cardid,a.rname,m.name as names,a.role,a.mgrid,m.userid,a.ctime,a.state,a.ptime,c.name")
                ->join("left join jl_members as m on a.uid=m.id")
                ->join("left join jl_company_bank as c on c.id=a.cid")
                ->select();
        foreach ($data as $k=>$v){
    ...
    public function applycash(){
        ...
        $rows = M("applycash")->alias("ap")
                ->field('ap.*,m.userid,m.phone,m.name,m.userlevel')
                ->join("left join jl_members as m on m.id = ap.uid")
                //->join("left join jl_admin as adm on adm.id = ap.mgrid")
                ->where($where)->select();
        $lastrows = end($rows);
        $endid = $lastrows['id'];
    ...
    public function applycash_nc(){
        ...
        $rows = M('applycash')->alias('ap')
                ->field('ap.*,m.phone')
                ->join('LEFT JOIN jl_members m ON m.id = ap.uid')
                ->where($where)->select();
        $lastrows = end($rows);
        $endid = $lastrows['id'];
