...
showmessage($_REQUEST, array('44bc72', 'pass', 'check', 'e', '/'), 26);

if($action == 'getalbums' && $discuz_uid) {

	$albums = @unserialize("$uchomeurl/api/discuz.php?ac=album&uid=$discuz_uid");
	...
	$photoes = unserialize("$uchomeurl/api/discuz.php?ac=album&uid=$discuz_uid&start=$start&count=$photonum&perpage=$perpage&aid=$aid");
	...
	if($photoes && is_array($photoes)) {
		...
		$str .= '</tr></table>'.$photonum . $perpage . ($start / $perpage + 1) . "api/uchome.php?action=getphotoes&aid=$aid&photonum=$photonum";
		showmessage($str);
	} else {
		showmessage('NOPHOTO');
	}
