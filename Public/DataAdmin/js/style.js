 ...
 * title: 头部的信息
*/
function popups (type, model, text, title){
	switch(type){
		case 1: //网页顶部下拉提示
			$(".popup").html(text);
...
$(function(){
  ...
  $('.sidebar-fold').click(function(){
      if($('.left-main').hasClass('left-full')){
        $('.left-main').removeClass('left-full').addClass('left-off');
        $('.right-product').removeClass('right-full').addClass('right-off');
      }else{
        $('.left-main').removeClass('left-off').addClass('left-full');
        $('.right-product').removeClass('right-off').addClass('right-full');
      }
  })
  ...
  $('.subNav').click(function(){
      if($(this).hasClass('sublist-down')){
        ...
        $(this).children('.title-icon').removeClass('glyphicon-chevron-down').addClass('glyphicon-chevron-up');
        $(this).siblings('.navContent').slideUp(200);
      }else{
        $(this).removeClass('sublist-up').addClass('sublist-down');
        $(this).children('.title-icon').removeClass('glyphicon-chevron-up').addClass('glyphicon-chevron-down');
        $(this).siblings('.navContent').slideDown(200);

        $(this).parent().siblings('.sBox').find('.navContent').slideUp(200);
        $(this).parent().siblings('.sBox').find('.subNav').removeClass('sublist-down').addClass('sublist-up')
        .find('.title-icon').removeClass('glyphicon-chevron-down').addClass('glyphicon-chevron-up');
      }
  })
  ...
  $('.changecolor li').click(function(){
      ...
      if(skin){
        $('.bodyskin[title!="'+ skin +'"]').attr('disabled',true);
        $('.bodyskin[title="'+ skin +'"]').attr('disabled',false);
      }
  })
  //全选/反选
  $('.selectAll').click(function(){
    $('.selectAll-item').each(function(){
...
//锁定用户界面
$(function(){
        $('#windowLock').click(user_window_lock);      
        $('#unlock_btn').click(function(){
            var pwd = $.trim($('#lock_pwd').val());
            if(pwd == ''){
              msg('请输入密码');
              return false;
            }
            user_window_unlock(pwd);
        })

        $('#windowLockModal').on('shown.bs.modal', function () {
            $('#lock_pwd').focus();
        })
})

function show_lock(){
    $('#windowLockModal').siblings('div,nav').css('webkitFilter', 'blur(2px)');        
    $('#windowLockModal').modal({
        keyboard: false,
        backdrop: 'static'
    });
    $('#lock_pwd').val('');
    $('#unlock_btn').removeClass('btn-success').addClass('btn-primary').text('提交解锁');
    $('#windowLockModal').modal('show');
}

function hide_lock(){
    $('#windowLockModal').modal('hide');
    $('#windowLockModal').siblings('div,nav').css('webkitFilter', 'blur(0px)');
}

function user_window_lock(){
    var params = {
        url : lockUrl,
        params : {},
...
function user_window_unlock(pwd){
    var params = {
        url : unlockUrl,
        params : {pwd : pwd},
        callback : function(data){
            var msgCallBack;
            if(data.status == 1){
                $('#unlock_btn').removeClass('btn-primary').addClass('btn-success').text('解锁成功');
                setTimeout(function(){
                hide_lock();
                },800);
            }else{
...
function no_auth_lock(){
    $('#noAuthModal').modal({
        keyboard: false,
        backdrop: 'static'
    });
    $('#noAuthModal').on('show.bs.modal', function(){
        $('#noAuthModal').siblings('div,nav').each(function(){
            $(this).css('webkitFilter', 'blur(1px)');
        });
    })
